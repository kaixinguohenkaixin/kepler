...
CCPATH=github.com/hyperledger/fabric/peer/chaincode/crosscode
...
installChaincode () {
    ...
    ORG=$2
	setGlobals $PEER $ORG
	peer chaincode install -n mycc -v 1.0 -p $CCPATH >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer1.example.com:7050 -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["init"]}' >&log.txt
	else
		peer chaincode instantiate -o orderer1.example.com:7050 --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["init"]}' >&log.txt
	fi
	res=$?
	cat log.txt
